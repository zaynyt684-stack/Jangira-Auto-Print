...
function cors(extra = {}) {
  return {
    ...
    "Access-Control-Allow-Headers": "Content-Type, Authorization, X-Agent-Token, X-Operator-Token",
...
// Operator authentication intentionally accepts the same AGENT_TOKEN through
// X-Agent-Token as well as the newer operator-specific headers. This keeps the
// private operator page compatible with older deployed Worker versions while
// retaining OPERATOR_SECRET support when it is configured.
function operatorOk(request, env) {
  const expected = env.OPERATOR_SECRET || env.AGENT_TOKEN;
  if (!expected) return false;
  const supplied =
...
function requestFilter(id) {
  ...
  if (/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value)) return `id=eq.${encodeURIComponent(value)}`;
  if (/^\d+$/.test(value)) return `request_number=eq.${encodeURIComponent(value)}`;
  return null;
}
...
async function getServiceStatus(_request, env) {
  try {
    const state = await serviceStatus(env);
    return json({ online: state.online, updatedAt: state.updatedAt });
  } catch (e) {
    return json({ online: false, error: "Service status is not configured." }, 503);
  }
}
async function setServiceStatus(request, env) {
  ...
  try {
    ...
    const rows = await supaJson(env, "/rest/v1/service_status?on_conflict=id", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Prefer: "resolution=merge-duplicates,return=representation"
      },
      body: JSON.stringify({ id: 1, online: body.online, updated_at: now })
    });
    return json({ online: rows?.[0]?.online === true, updatedAt: rows?.[0]?.updated_at || now });
  } catch (e) {
    return json({
      error: "Service status storage is not configured.",
...
async function createPrintRequest(request, env) {
  try {
    if (!(await serviceStatus(env)).online) return json({ error: "Jangira AutoPrint is currently closed." }, 503);
  } catch (_) {
    return json({ error: "Jangira AutoPrint is currently unavailable." }, 503);
  }
  ...
  return json({ id, requestNumber, fileName: originalFileName, pageCount, selectedPageCount: selectedCount, copies, sides, orientation, paperSize, colorMode, amount, paymentStatus: "PENDING", status: "RECEIVED" }, 201);
}
...
async function agentDecision(request, env, id) { if (!agentOk(request, env)) return json({ error: "Unauthorized agent" }, 401); const body = await request.json().catch(() => ({})); const order = await getRequest(env, id); if (!order) return json({ error: "Request not found" }, 404); if (order.status !== "REVIEWING") return json({ error: "Request must be in REVIEWING state." }, 409); const action = String(body.action || "").toLowerCase(); if (action === "reject") { const reason = String(body.reason || "Rejected by operator").slice(0, 500); await updateRequest(env, order.id, { status: "REJECTED", rejection_reason: reason, operator_note: String(body.note || "").slice(0, 1000), rejected_at: new Date().toISOString() }); await event(env, order.id, "REJECTED", reason, operatorNameSafe(body)); return json({ ok: true, status: "REJECTED" }); } if (action === "approve") { await updateRequest(env, order.id, { status: "QUEUED_FOR_PRINTING", operator_note: String(body.note || "").slice(0, 1000), approved_at: new Date().toISOString() }); await event(env, order.id, "APPROVED", "Payment verified and request approved", operatorNameSafe(body)); return json({ ok: true, status: "QUEUED_FOR_PRINTING" }); } return json({ error: "action must be approve or reject" }, 400); }
function operatorNameSafe(body) { return String(body.operatorName || "Jangira Operator").slice(0, 100); }
async function agentStatus(request, env, id) { if (!agentOk(request, env)) return json({ error: "Unauthorized agent" }, 401); const body = await request.json().catch(() => ({})); const order = await getRequest(env, id); if (!order) return json({ error: "Request not found" }, 404); const next = String(body.status || "").toUpperCase(); if (!["PRINTING", "COMPLETED", "FAILED", "CANCELLED"].includes(next)) return json({ error: "Invalid print status" }, 400); const patch = { status: next, updated_at: new Date().toISOString() }; if (next === "PRINTING") patch.print_started_at = new Date().toISOString(); if (next === "COMPLETED") { patch.completed_at = new Date().toISOString(); patch.printed_copies = order.copies || 1; } if (next === "FAILED") { patch.print_attempts = Number(order.print_attempts || 0) + 1; patch.failure_reason = String(body.reason || "Print failed").slice(0, 500); } await updateRequest(env, order.id, patch); await event(env, order.id, next, String(body.message || next), "agent"); return json({ ok: true, status: next }); }
async function downloadFile(request, env, id, kind) { if (!agentOk(request, env)) return json({ error: "Unauthorized agent" }, 401); const order = await getRequest(env, id); if (!order) return json({ error: "Request not found" }, 404); const path = kind === "original" ? order.file_path : kind === "edited" ? order.edited_file_path : order.final_file_path; if (!path) return json({ error: "Requested file is not available." }, 404); const r = await supa(env, `/storage/v1/object/${BUCKET}/${path}`); if (!r.ok) return json({ error: "File download failed.", details: (await r.text()).slice(0, 300) }, 502); const headers = new Headers(cors()); headers.set("Content-Type", order.mime_type || "application/octet-stream"); headers.set("Content-Disposition", `attachment; filename="${cleanName(order.original_file_name)}"`); return new Response(r.body, { status: 200, headers }); }
async function cleanup(env) { try { const rows = await supaJson(env, `/rest/v1/print_requests?delete_after=lt.${encodeURIComponent(new Date().toISOString())}&select=id,file_path,edited_file_path,final_file_path&limit=100`); for (const row of rows || []) { for (const path of [row.file_path, row.edited_file_path, row.final_file_path].filter(Boolean)) await supa(env, `/storage/v1/object/${BUCKET}/${path}`, { method: "DELETE" }).catch(() => {}); await supa(env, `/rest/v1/print_requests?id=eq.${encodeURIComponent(row.id)}`, { method: "DELETE" }).catch(() => {}); } } catch (_) {} }
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === "/api/print-request" && request.method === "POST") return createPrintRequest(request, env);
      ...
      const payment = url.pathname.match(/^\/api\/print-requests\/([^/]+)\/payment$/); if (payment && request.method === "POST") return submitPayment(request, env, payment[1]);
      if (url.pathname === "/api/agent/queue" && request.method === "GET") return agentQueue(request, env);
      const claim = url.pathname.match(/^\/api\/agent\/requests\/([^/]+)\/claim$/); if (claim && request.method === "POST") return agentClaim(request, env, claim[1]);
      const decision = url.pathname.match(/^\/api\/agent\/requests\/([^/]+)\/decision$/); if (decision && request.method === "POST") return agentDecision(request, env, decision[1]);
      const printStatus = url.pathname.match(/^\/api\/agent\/requests\/([^/]+)\/status$/); if (printStatus && request.method === "POST") return agentStatus(request, env, printStatus[1]);
      const file = url.pathname.match(/^\/api\/agent\/requests\/([^/]+)\/file\/(original|edited|final)$/); if (file && request.method === "GET") return downloadFile(request, env, file[1], file[2]);
