...
function selectedPagesCount(value, pageCount) {
  ...
  for (const part of v.split(",").map(x => x.trim()).filter(Boolean)) {
    if (/^\d+$/.test(part)) { const n = Number(part); if (n < 1 || n > pageCount) return null; pages.add(n); continue; }
    const m = part.match(/^(\d+)\s*-\s*(\d+)$/); if (!m) return null;
    let a = Number(m[1]), b = Number(m[2]); if (a > b) [a, b] = [b, a]; if (a < 1 || b > pageCount) return null;
...
async function agentDecision(request, env, id) { if (!agentOk(request, env)) return json({ error: "Unauthorized agent" }, 401); const body = await request.json().catch(() => ({})); const order = await getRequest(env, id); if (!order) return json({ error: "Request not found" }, 404); if (order.status !== "REVIEWING") return json({ error: "Request must be in REVIEWING state." }, 409); const action = String(body.action || "").toLowerCase(); if (action === "reject") { const reason = String(body.reason || "Rejected by operator").slice(0, 500); await updateRequest(env, order.id, { status: "REJECTED", rejection_reason: reason, operator_note: String(body.note || "").slice(0, 1000), rejected_at: new Date().toISOString() }); await event(env, order.id, "REJECTED", reason, order.operator_name || "operator"); return json({ id: order.id, requestNumber: order.request_number, status: "REJECTED" }); } if (action !== "approve") return json({ error: "action must be approve or reject" }, 400); const patch = { status: "QUEUED_FOR_PRINTING", operator_note: String(body.note || "").slice(0, 1000), edited_file_path: body.editedFilePath || order.edited_file_path || null, final_file_path: body.finalFilePath || order.final_file_path || null, approved_at: new Date().toISOString() }; const updated = await updateRequest(env, order.id, patch); await event(env, order.id, "APPROVED", "Request approved and queued for printing", order.operator_name || "operator"); return json(updated?.[0] || { id: order.id, requestNumber: order.request_number, status: patch.status }); }
async function agentStatus(request, env, id) { if (!agentOk(request, env)) return json({ error: "Unauthorized agent" }, 401); const body = await request.json().catch(() => ({})); const order = await getRequest(env, id); if (!order) return json({ error: "Request not found" }, 404); const allowed = ["PRINTING", "COMPLETED", "FAILED", "CANCELLED"]; const status = String(body.status || "").toUpperCase(); if (!allowed.includes(status)) return json({ error: "Invalid print status" }, 400); const patch = { status }; if (status === "PRINTING") patch.printing_started_at = new Date().toISOString(); if (status === "COMPLETED") patch.completed_at = new Date().toISOString(); if (status === "FAILED") patch.last_error = String(body.error || "Print failed").slice(0, 1000); if (body.printedCopies != null) patch.printed_copies = Math.max(0, Number(body.printedCopies) || 0); if (body.printAttempts != null) patch.print_attempts = Math.max(0, Number(body.printAttempts) || 0); const updated = await updateRequest(env, order.id, patch); await event(env, order.id, status, body.message || status, order.operator_name || "agent"); return json(updated?.[0] || { id: order.id, requestNumber: order.request_number, status }); }
async function downloadFile(request, env, id, kind = "original") { if (!agentOk(request, env)) return json({ error: "Unauthorized agent" }, 401); const order = await getRequest(env, id); if (!order) return json({ error: "Request not found" }, 404); const path = kind === "final" ? order.final_file_path : kind === "edited" ? order.edited_file_path : order.file_path; if (!path) return json({ error: "File not available" }, 404); const r = await supa(env, `/storage/v1/object/${BUCKET}/${path}`); if (!r.ok) return json({ error: "File download failed" }, 502); return new Response(r.body, { status: 200, headers: cors({ "Content-Type": r.headers.get("Content-Type") || "application/octet-stream", "Content-Disposition": `attachment; filename="${cleanName(order.original_file_name)}"` }) }); }
async function cleanup(env) { const rows = await supaJson(env, `/rest/v1/print_requests?status=eq.COMPLETED&delete_after=lt.${encodeURIComponent(new Date().toISOString())}&select=id,file_path,edited_file_path,final_file_path&limit=100`); for (const row of rows || []) { for (const path of [row.file_path, row.edited_file_path, row.final_file_path].filter(Boolean)) await supa(env, `/storage/v1/object/${BUCKET}/${path}`, { method: "DELETE" }).catch(() => {}); await updateRequest(env, row.id, { file_path: null, edited_file_path: null, final_file_path: null, original_deleted_at: new Date().toISOString(), edited_deleted_at: new Date().toISOString(), cleanup_completed_at: new Date().toISOString() }).catch(() => {}); } return rows?.length || 0; }
...
export default { async fetch(request, env) { if (request.method === "OPTIONS") return new Response(null, { status: 204, headers: cors() }); const url = new URL(request.url); try {
  ...
  const s = url.pathname.match(/^\/api\/print-requests\/([^/]+)$/); if (s && request.method === "GET") { const row = await getRequest(env, s[1]); if (!row) return json({ error: "Request not found" }, 404); const pageCount = Math.max(1, Number(row.page_count || 1)); const selectedPageCount = selectedPagesCount(row.page_range || "all", pageCount) || pageCount; return json({ id: row.id, requestNumber: row.request_number, fileName: row.original_file_name, pageCount, selectedPageCount, copies: row.copies, sides: row.sides, orientation: row.orientation, paperSize: row.paper_size, colorMode: row.color_mode, pageRange: row.page_range || "all", amount: row.payment_amount, paymentStatus: row.payment_status, status: row.status, createdAt: row.created_at, updatedAt: row.updated_at, queuePosition: null }); }
...
} catch (e) { return json({ error: "Backend error", details: String(e?.message || e).slice(0, 500) }, 500); } }, async scheduled(_controller, env) { await cleanup(env); } };